...
resource "aws_network_interface" "gpu_node_eni" {
  count           = 2
  ...
  tags = merge(var.common_tags, {
    Name = "${var.project_name}-gpu-node-eni-${count.index}"
  })
}
...
resource "aws_instance" "gpu_node" {
  ...
  network_interface {
    network_interface_id = aws_network_interface.gpu_node_eni[count.index].id
    device_index         = 0
  }
  ...
  tags = merge(var.common_tags, {
    Name = "${var.project_name}-gpu-node-${count.index}"
  })
}
